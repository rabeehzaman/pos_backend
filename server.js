...
// ========== LASTSOLD PRICING STRATEGY API ENDPOINTS ==========

// LastSold prices storage
const lastSoldPricesFile = path.join(__dirname, 'data', 'lastSoldPrices.json');
...
// Helper function to load LastSold prices
function loadLastSoldPrices() {
    try {
        if (fs.existsSync(lastSoldPricesFile)) {
            const fileContent = fs.readFileSync(lastSoldPricesFile, 'utf8');
            return fileContent ? JSON.parse(fileContent) : {};
        }
        return {};
    } catch (error) {
        console.error('Error loading LastSold prices:', error);
        return {};
    }
}

// Helper function to save LastSold prices
function saveLastSoldPrices(prices) {
    try {
        fs.writeFileSync(lastSoldPricesFile, JSON.stringify(prices, null, 2));
        return true;
    } catch (error) {
        console.error('Error saving LastSold prices:', error);
        return false;
    }
}

// Save LastSold price
app.post('/api/last-sold-prices/save', (req, res) => {
    console.log('\n========== SAVING LASTSOLD PRICE ==========');
    
    try {
        const { itemId, unit, branchId, taxMode, price, itemName, branchName } = req.body;
        
        // Validate required fields
        if (!itemId || !unit || !branchId || !taxMode || price === undefined) {
            return res.status(400).json({
                error: 'Missing required fields',
                required: ['itemId', 'unit', 'branchId', 'taxMode', 'price'],
                success: false
            });
        }
        ...
        // Create unique key: itemId_unit_branchId_taxMode
        const priceKey = `${itemId}_${unit}_${branchId}_${taxMode}`;
        
        // Load existing prices
        const prices = loadLastSoldPrices();
        
        // Save price with metadata
        prices[priceKey] = {
            price: numPrice,
        ...
        // Save to file
        const saved = saveLastSoldPrices(prices);
        
        if (saved) {
            console.log(`[LASTSOLD] Saved price for ${priceKey}: ${numPrice}`);
            res.json({
                success: true,
                message: 'LastSold price saved successfully',
                key: priceKey,
                price: numPrice
            });
        } else {
        ...
        console.error('❌ LastSold price save failed:', error);
        res.status(500).json({
...
// Get LastSold price for a specific item/unit/branch/taxMode combination
app.get('/api/last-sold-prices/get', (req, res) => {
    console.log('\n========== GETTING LASTSOLD PRICE ==========');
    
    try {
        const { itemId, unit, branchId, taxMode } = req.query;
        
        // Validate required parameters
        if (!itemId || !unit || !branchId || !taxMode) {
            return res.status(400).json({
                error: 'Missing required parameters',
                required: ['itemId', 'unit', 'branchId', 'taxMode'],
                success: false
            });
        }
        ...
        if (priceData) {
            console.log(`[LASTSOLD] Found price for ${priceKey}: ${priceData.price}`);
            res.json({
                success: true,
                found: true,
                key: priceKey,
            ...
            console.log(`[LASTSOLD] No price found for ${priceKey}`);
            res.json({
                success: true,
                found: false,
                key: priceKey
        ...
        console.error('❌ LastSold price get failed:', error);
        res.status(500).json({
            error: 'Internal server error',
            success: false,
            details: error.message
...
// Bulk get LastSold prices
app.post('/api/last-sold-prices/bulk', (req, res) => {
    console.log('\n========== BULK GETTING LASTSOLD PRICES ==========');
    
    try {
        const { items, branchId, taxMode } = req.body;
        
        // Validate required fields
        if (!items || !Array.isArray(items) || !branchId || !taxMode) {
            return res.status(400).json({
                error: 'Missing required fields',
                required: ['items (array)', 'branchId', 'taxMode'],
                success: false
            });
        }
        
        // Load all prices
        const allPrices = loadLastSoldPrices();
        const result = {};
        
        // Get price for each item/unit combination
        items.forEach(item => {
            if (item.itemId && item.unit) {
                const priceKey = `${item.itemId}_${item.unit}_${branchId}_${taxMode}`;
                const priceData = allPrices[priceKey];
                
                if (priceData) {
                    result[priceKey] = priceData;
                }
            }
        });
        
        console.log(`[LASTSOLD] Found ${Object.keys(result).length} prices out of ${items.length} requested items`);
        
        res.json({
            success: true,
            prices: result,
            found_count: Object.keys(result).length,
            requested_count: items.length
        });
        
    } catch (error) {
        console.error('❌ LastSold bulk prices get failed:', error);
        res.status(500).json({
            error: 'Internal server error',
            success: false,
            details: error.message
...
// Clear LastSold prices (optional - for settings page)
app.delete('/api/last-sold-prices/clear', (req, res) => {
    console.log('\n========== CLEARING LASTSOLD PRICES ==========');
    ...
    try {
        ...
        if (confirm !== true) {
            return res.status(400).json({
                ...
                message: 'Set confirm: true to clear prices',
                success: false
            });
        }
        
        if (branchId) {
            // Clear prices for specific branch
            const prices = loadLastSoldPrices();
            const branchPrefix = `_${branchId}_`;
            let deletedCount = 0;
            
            Object.keys(prices).forEach(key => {
                if (key.includes(branchPrefix)) {
                    delete prices[key];
                    deletedCount++;
                }
            });
            ...
            if (saved) {
                console.log(`[LASTSOLD] Cleared ${deletedCount} prices for branch ${branchId}`);
                res.json({
                    success: true,
                    message: `Cleared ${deletedCount} prices for branch`,
                    deletedCount,
                    branchId
                });
            } else {
                res.status(500).json({
                    error: 'Failed to save after clearing',
                    success: false
                });
            }
        } else {
            // Clear all prices
            const saved = saveLastSoldPrices({});
            
            if (saved) {
                console.log('[LASTSOLD] Cleared all prices');
                res.json({
                    success: true,
                    message: 'All LastSold prices cleared'
                });
            } else {
                res.status(500).json({
                    error: 'Failed to clear prices',
                    success: false
                });
            }
        }
        ...
        console.error('❌ LastSold prices clear failed:', error);
        res.status(500).json({
            error: 'Internal server error',
            success: false,
            details: error.message
...
// Get LastSold prices statistics (for settings page)
app.get('/api/last-sold-prices/stats', (req, res) => {
    try {
        const prices = loadLastSoldPrices();
            ...
            if (parts.length >= 3) {
                ...
                if (!branchStats[branchId]) {
                    branchStats[branchId] = {
                        count: 0,
                        branchName: prices[key].branchName || 'Unknown'
                    };
                }
        ...
        res.json({
            ...
            totalCount,
            branchStats,
            lastUpdated: fs.existsSync(lastSoldPricesFile) 
                ? fs.statSync(lastSoldPricesFile).mtime.toISOString()
                : null
        });
        
    } catch (error) {
        console.error('❌ LastSold stats failed:', error);
        res.status(500).json({
            error: 'Internal server error',
            success: false,
            details: error.message
